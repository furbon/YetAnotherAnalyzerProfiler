...
テーマはWPFのDynamicResourceによる共通パレットです。既定のAutoはWindowsの
`AppsUseLightTheme`を参照し、読み取り不能時はライトへ安全にフォールバックします。明示的な
ライト／ダーク指定は同じリソースキーを差し替えるため、再起動せず全コントロールへ反映されます。
