...
> [!NOTE]
> これは [英語版](../../SUPPORT.md) の公式日本語訳です。英語版を正本とし、内容が異なる場合は英語版を優先します。

質問、バグ報告、機能提案、Pull Requestは英語または日本語で受け付けます。応答と翻訳は
メンテナーの対応可能な範囲で行うベストエフォートであり、応答時間や日本語対応を保証するものではありません。
...
SDK、OS、ランナーの具体的な検証範囲は [テスト方針（英語）](../testing.md) を参照してください。
...
`.slnx`、private feed、isolated出力、診断コードについては [利用ガイド](usage.md) と
[トラブルシュート](troubleshooting.md) も参照してください。
