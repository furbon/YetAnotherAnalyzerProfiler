...
なければ `Release`、`Debug`、その他の構成名のアルファベット順で自動選択します。画面下部には
「測定可能」または測定開始できない理由と、検出・測定処理の状態を常に表示します。

テーマは起動時にWindowsのアプリテーマへ自動追従します。上部のテーマ選択で「自動」、
「ライト」、「ダーク」をいつでも切り替えられます。履歴場所、保持件数、clean、分離出力、
